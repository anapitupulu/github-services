...
In order to run the services, these variables *have to be defined* in `.env` (or set as environment variables):
...
If for some reason it's not able to connect to the database, the service will quit immediately.
...
- index.ts - the main file to setup the Express server, Router and logger  
...
To run the test, you can execute `yarn test`.
...
Unit tests can be found in `src/__tests__/e2e.spec.ts`.
